...
int BuildSampledLCPArray(char *text, unsigned int textsize, unsigned char *lcparray, int minlcp, int verbose, unsigned int first_sa_position, int argCircular){
	unsigned int textpos, prevtextpos, original_textpos;
	...
	//edited
	if(argCircular == 1)
		prevtextpos=first_sa_position; // the 0-th BWT position corresponds to the terminator symbol at the last text position
	else
		prevtextpos=textsize;	
	textpos=0;
	for(bwtpos=1;bwtpos<=bwtLength;bwtpos++){ // all BWT positions and one fake next-to-last position to fill last position
		...
		if(bwtpos!=bwtLength){
			#ifdef BUILDLCP
			textpos=FMI_PositionInText(bwtpos, argCircular);
			original_textpos = textpos;
			...
			if(argCircular == 1)
				while(1){
				//	printf("\ntextpos=%d, prevtextpos=%d, lcp=%d\n",textpos,prevtextpos, lcp);
					if(textpos == textsize) {
						textpos=0;
						bottomstring=(char *)(text);
					}
					
					if(prevtextpos == textsize) {
						prevtextpos=0;
						topstring=(char *)(text);
					}
					//((*topstring)==(*bottomstring)) && (prevtextpos!=textsize)
					if((*topstring)!=(*bottomstring)) break;
					topstring++;
					bottomstring++;
					lcp++;
					prevtextpos++;
			...
			if( lcp == (int)UCHAR_MAX ){ // if it is a large (>255) lcp, calculate its value
				prevtextpos=FMI_PositionInText((bwtpos-1), argCircular);
				textpos=FMI_PositionInText(bwtpos, argCircular);
