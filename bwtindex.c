...
unsigned int BitsSetCount( unsigned int bitArray ){
	/*
	while( bitArray ){
...
unsigned int FMI_PositionInText( unsigned int bwtpos , int argCircular){
	...
	unsigned int textPos = (Index[( bwtpos >> SAMPLEINTERVALSHIFT )].textPositionSample) + addpos;
	if ( textPos >= bwtSize ) textPos = textPos - bwtSize;
	return ( textPos);
}
...
void PrintBWT(unsigned int *letterStartPos, int argCircular){
	unsigned int i, n, p;
	printf("%u {", bwtSize);
	...
	for (i = 0; i < bwtSize; i++){ // position in BWT
		p = FMI_PositionInText(i, argCircular);
...
//edited from
// Structure to store the ranks of suffixes
struct suffix
{
    long index;
...
int compareSuffixes(const void *a, const void *b) {
    return strcmp((const char*)((struct suffix *)a)->suffix, (const char*)((struct suffix *)b)->suffix);
}

int compareCircularSuffix(const void *a, const void *b) {
    return strcmp((const char*)a, (const char*)b);
}

int letterToSAbucketPos(int letter, int st)
{   
    if(st == 1) return letter;
...
void printSA(struct suffix *suffixArray, long length)
{
    printf("\nSA: ");
    printf(" {");
    for (int x = 0; x < length; x++)
    {
        printf("%02li ", suffixArray[x].index);
    }
    printf("}");
}

struct SuffixBuckets * initSuffixBucket(struct SuffixBuckets *buckets, long alphabetSize)
{
    for (long i = 0; i < alphabetSize; i++)
    {
        if (i > 0)
            buckets[i].start = buckets[i - 1].pointer_L + 1;
        else
            buckets[i].start = 0;
        buckets[i].pointer_S = buckets[i].start;
        buckets[i].pointer_L = buckets[i].pointer_S + buckets[i].size - 1;
    }

    return buckets;
}

int isLMS(unsigned i, int *t)
{
    return (t[i - 1] > t[i]) ? 1 : 0;
}

struct SuffixBuckets putS(struct suffix *suffixArray, struct SuffixBuckets bucket, unsigned char **suffixPtr, long index)
{
    suffixArray[bucket.pointer_L].index = index;
    suffixArray[bucket.pointer_L].suffix = *suffixPtr;
    bucket.pointer_L--;
    return bucket;
}

struct SuffixBuckets putL(struct suffix *suffixArray, struct SuffixBuckets bucket, unsigned char **suffixPtr, long index)
{
    suffixArray[bucket.pointer_S].index = index;
    suffixArray[bucket.pointer_S].suffix = *suffixPtr;
    bucket.pointer_S++;
    return bucket;
}

unsigned char* getPrefix(const unsigned char *byteArray, size_t i, size_t length){
    // Calculate the new size of the array after removing the last 10 elements
   

    // Create a new byte array with the updated size
    unsigned char* newByteArray = malloc((length-1) * sizeof(unsigned char));
    size_t ite = 0;
    for (size_t y = i; y < length-1; y++) {
        
        newByteArray[ite] = byteArray[y];
        ite++;
    }

    for (size_t z = 0; z < i; z++) {
        
        newByteArray[ite] = byteArray[z];
        ite++;
    }
...
void classify_L_S(const unsigned char *byteArray, int *t, size_t length, struct SuffixBuckets *buckets, long alphabetSize, int st)
{
    buckets[0].size = 1;
    for (int i = length - 2; i >= 0; i--)
    {
        
        buckets[letterToSAbucketPos(byteArray[i], st)].size++;
        buckets = initSuffixBucket(buckets, alphabetSize);
        if (byteArray[i] < byteArray[i + 1])
            t[i] = 0; // S
        else if (byteArray[i] > byteArray[i + 1])
            t[i] = 1; // T
        else
            t[i] = t[i + 1];
    }
...
struct suffix * orderL(const unsigned char *byteArray, struct suffix *suffixArray, size_t length , size_t index ){
     for (size_t i = 1; i < length; i++){
         if(suffixArray[i].index != -1 && suffixArray[i].index > index){
             size_t i_temp = i;
             while(i_temp < length -1 && suffixArray[i_temp+1].index != -1 && (compareCircularSuffix(getPrefix(byteArray, suffixArray[i_temp].index, length), getPrefix(byteArray, suffixArray[i_temp+1].index, length))) > 0){
                struct suffix temp_suffix = suffixArray[i_temp];
                suffixArray[i_temp]  = suffixArray[i_temp + 1];
                suffixArray[i_temp + 1] = temp_suffix;
                i_temp++;
                if(suffixArray[i_temp].index > index) i--;
             }
...
struct suffix * induce_L_S(struct suffix *suffixArray, const unsigned char *byteArray, int *t, size_t length, struct SuffixBuckets *buckets, int st, int mode)
{
    for (size_t i = 0; i < length; i++)
    {
        const long pos = suffixArray[i].index - 1;
        if (pos >= 0 && t[pos] == 1)
        {
            unsigned char *suffixPtr = (unsigned char *)(byteArray + pos);
            buckets[letterToSAbucketPos(byteArray[pos], st)] = putL(suffixArray, buckets[letterToSAbucketPos(byteArray[pos], st)], &suffixPtr, pos);
        }
    }

    if(mode == 1){
        //ordenar todos os [L's] antecedentes ao primeiro [S] a contar do  fim
        size_t index = length-2;
        while(t[index] == 1)index--;
        orderL(byteArray, suffixArray, length , index );
    }
    ...
    for (size_t i = length - 1; i > 0; i--)
    {
        ...
        {
            unsigned char *suffixPtr = (unsigned char *)(byteArray + pos);
            buckets[letterToSAbucketPos(byteArray[pos], st)] = putS(suffixArray, buckets[letterToSAbucketPos(byteArray[pos], st)], &suffixPtr, pos);
        }
    }
...
struct suffix * first_induce(const unsigned char *byteArray, int *t, size_t length, struct suffix *suffixArray, struct SuffixBuckets *buckets, long alphabetSize, int st, int mode)
{
    buckets = initSuffixBucket(buckets, alphabetSize);
    struct suffix *lms = (struct suffix *)malloc(sizeof(struct suffix) * 1);
    int p=0;
    for (size_t i = length - 1; i > 0; i--)
        if (isLMS(i, t))
        {   
            p++;
            lms = (struct suffix *)realloc(lms, sizeof(struct suffix) * p);
            lms[p-1].index = i;
            lms[p-1].suffix = getPrefix(byteArray, i, length);
        }

    qsort(lms, p, sizeof(struct suffix), compareSuffixes);

    for(int i = p-1; i >=0; i--){
        buckets[letterToSAbucketPos(byteArray[lms[i].index], st)] = putS(suffixArray, buckets[letterToSAbucketPos(byteArray[lms[i].index], st)], &(lms[i].suffix), lms[i].index);
    }

    buckets = initSuffixBucket(buckets, alphabetSize);
    suffixArray = induce_L_S(suffixArray, byteArray, t, length, buckets, st, mode);
    return suffixArray;
}

struct suffix writeBWTToFile(const unsigned char *byteArray, struct suffix *suffixArray, size_t length, const char *file_name, int mode) {
    char out_filename[100];
    snprintf(out_filename, sizeof(out_filename), "%s.bwt", file_name);
	struct suffix bwt; 
	
	bwt.suffix = malloc((length-1)*sizeof(unsigned char));
    ...
    if(mode == 1)
        for (size_t i = 1; i < length; i++) {
                if (suffixArray[i].index > 0){
					letra = byteArray[suffixArray[i].index - 1];
					if( i == 1)
					bwt.index = suffixArray[i].index;
				}
                else{
					letra = byteArray[length - 2];
					if( i == 1)
						bwt.index = 0;
				}    
				bwt.suffix[i-1] = letra;
		...
		for (size_t i = 0; i < length; i++) {
				if (suffixArray[i].index > 0){
					letra = byteArray[suffixArray[i].index - 1];
					if( i == 1)
					bwt.index = suffixArray[i].index - 1;
				}	
				else{
					letra = byteArray[length - 1];
					if( i == 1)
						bwt.index = length - 1;
				}	
				bwt.suffix[i] = letra;
...
struct suffix constructSuffixArray(unsigned char *byteArray, size_t length, long alphabetSize, int st, const char *file_name, int mode)
{   
    int *t = (int *)malloc(length * sizeof(int));
    struct suffix *suffixes = (struct suffix *)malloc(sizeof(struct suffix) * length);
    for (size_t i = 0; i < length; i++)
    {
        suffixes[i].index = -1;
    }

    struct SuffixBuckets *buckets = (struct SuffixBuckets *)malloc(sizeof(struct SuffixBuckets) * alphabetSize);
    classify_L_S(byteArray, t, length, buckets, alphabetSize, st);
    first_induce(byteArray, t, length, suffixes, buckets, alphabetSize, st, mode);
    return writeBWTToFile(byteArray, suffixes, length, file_name, mode);
...
struct returnBuildBwt FMI_BuildIndex(char **inputTexts, unsigned int *inputTextSizes, unsigned int inputNumTexts, unsigned char **lcpArrayPointer, char verbose, char * returning_text, int argCircular){
	...
	*/
	...
	//edited
    if(argCircular == 1){
        //make the sequence end on a L(arger) character than the first
        size_t n  = 0;
        for(n = bwtSize-2; n > 0; n--)
            if(text[n] < text[n-1])
                break;
            
        text = getPrefix(text, n, bwtSize);
        //text[bwtSize-1] = '$';
		printf("\n N = %zu\n", n);

		printf("\n text = %s\n", text);
		//circular sequence after preprocessing to better induce the SA
		FILE *file;
		file = fopen("_circular.fasta", "w");
		if (file == NULL) {
			perror("Error opening the file");
			struct returnBuildBwt re = {1,text};
			return re;
		}
		fprintf(file, "%s", text);
		fclose(file);
	...
	struct suffix bwt;
	if(argCircular == 1){
		bwt = constructSuffixArray(text, (size_t)bwtSize, 5, 0, "temp-bwt", 1);
		printf("suffix: %s\n", bwt.suffix);
		printf("bwt.index: %d\n", bwt.index);
		bwtSize--;
	}
	...
	for( n = 0 ; n < bwtSize ; n++ ){
		...
		if(argCircular == 0){
			...
			letterId = letterIds[bwt.suffix[n]];
		}
		//edited
		//letterId = GetPackedNumber(packedBwt,n);
		#endif
	...
	//edited pos in the SA (first element in bwt)
	if(argCircular == 1)
		textPos=bwt.index; // start with the position of the terminator char in the text
	else textPos=(bwtSize-1);	
	...
	free(letterStartPos);
	if(argCircular == 1){
		struct returnBuildBwt re = {bwt.index, text};
		return re;
	} else{
		struct returnBuildBwt re = {1,text};
		return re;
